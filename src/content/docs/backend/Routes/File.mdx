...
The function uses 'check_if_unpublished' function ## add link.
...
The function uses 'change_publish_status' function ## add link.
...
The function uses 'validate_dataframe' and 'bulk_insert_data' functions ## add link to check the data type and insert it to the db.
...
#### Expected Input
  ...
  The function recieves five arguments: 
  1. current_user - gets the user by its userame
  2. model_name - the data type from the frontend (aid or drops) 
  3. file - a file from the frontend
  4. startDate - a date from the frontend that determines the starting relevant date range 
  5. endDate - a date from the frontend that determines the ending relevant date range 
...
The function uses 'download_data_from_db' function ## add link.
...
The function uses 'delete_unpublished_rows' function ## add link that deletes all the rows with publish = 1.
...
###### This function recieves a request from the frontend and deletes data between the requested data range.  
...
The function uses 'delete_rows' function ## add link that deletes all requested rows.
